...
void IOManager::LoadImageToCanvas(PixelBuffer** display_buffer_) {
    ...
    PixelBuffer * newCanvas = image->loadimage
      (file_name_, height, width, (*display_buffer_)->getBackgroundColor());
    ...
    // InitializeBuffers(display_buffer_->getBackgroundColor(), width, height);
...
void IOManager::LoadImageToStamp(Stamp* stamp) {
  std::cout << "Load Stamp has been clicked for file " <<
    ...
    int width;
    PixelBuffer * newCanvas = image->loadimage
      (file_name_, height, width, ColorData(1, 1, 1));

    if (newCanvas == NULL) {
        std::cout << "ERROR LOADING IMAGE" << std::endl;
    }

    stamp->LoadStamp(newCanvas);
}

void IOManager::SaveCanvasToFile(PixelBuffer* display_buffer_) {
  std::cout << "Save Canvas been clicked for file " <<
      file_name_ << std::endl;

    ImageManager * image = new ImageManager();
    image->saveimage(file_name_, display_buffer_);
    }
}  /* namespace image_tools */
